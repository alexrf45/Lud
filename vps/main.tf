...
resource "aws_key_pair" "bounty_ssh_key" {
  key_name   = var.key_name
  public_key = file("~/.ssh/bounty.pub")
}

#reserves a AWS Elastic IP
resource "aws_eip" "bounty_eip" {
  vpc = true
}

#Associates Elastic IP to ec2 instance
resource "aws_eip_association" "bounty_eip_assoc" {
  instance_id   = aws_instance.bounty_instance.id
  allocation_id = aws_eip.bounty_eip.allocation_id
}

#creates ec2 instance
resource "aws_instance" "bounty_instance" {
  ami                         = var.ami
...
resource "aws_security_group_rule" "allow_ssh_inbound" {
  type              = "ingress"
  security_group_id = aws_security_group.bounty_sg.id
  description       = var.sg_description
